...
export function UsernameLogin({ onConnect }: UsernameLoginProps) {
  ...
  const validateUsername = (value: string): ValidationError | null => {
    ...
    if (value.length > 20) {
      return { field: 'username', message: 'Username must be no more than 20 characters long' };
    }
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          ...
          if (data.error.includes('Username')) {
            setErrors([{ field: 'username', message: data.error }]);
          } else if (data.error.includes('phone')) {
            setErrors([{ field: 'phone', message: data.error }]);
          } else {
            setErrors([{ field: 'general', message: data.error }]);
          }
        } else if (response.status === 429) {
          setErrors([{ field: 'general', message: 'Too many login attempts. Please wait a few minutes and try again.' }]);
        } else {
          setErrors([{ field: 'general', message: 'Authentication failed. Please check your details and try again.' }]);
        }
        return;
      }
          ...
          <CardContent>
                  ...
                  >
                    <Alert className="border-red-200 bg-red-50 dark:bg-red-900/20">
                      ...
                      <AlertDescription className="text-red-700 dark:text-red-300">
                        <strong>Authentication Failed:</strong> {getGeneralError()}
                      </AlertDescription>
                    </Alert>
                  </motion.div>
                )}
                ...
                >
                  {isLoading ? (
                    <>
                      ...
                      {isValidating ? 'Validating...' : 'Logging in...'}
                    </>
                  ) : (
                    <>
            ...
            <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <p className="text-xs text-blue-700 dark:text-blue-300 text-center">
                <strong>New here?</strong> Just enter a unique username to create your account and join the conversation!
              </p>
            </div>
          </CardContent>
        </Card>
